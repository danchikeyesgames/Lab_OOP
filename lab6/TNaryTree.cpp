...
template <class T>
TIterator<TreeItem<T>, T> TNaryTree<T>::begin() {
  return TIterator<TreeItem<T>, T>(root);
}

template <class T>
TIterator<TreeItem<T>, T> TNaryTree<T>::end() {
  return TIterator<TreeItem<T>, T>(nullptr);
}

#include "hexagon.h"
template class TNaryTree<hexagon>;
template std::ostream& operator<<(std::ostream& os, const TNaryTree<hexagon>& tree);