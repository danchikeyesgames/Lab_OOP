...
polynomial_t *polynomial_create(size_t size) {
    ...
    polynomial->degree = 0;
    polynomial->data = (long long *)malloc(sizeof(long long) * size);
...
int polynomial_insert_digit(polynomial_t **polynomial, long long digit) {
...
int print_polynomial(polynomial_t **polynomial) {
    ...
    for (size_t i = 0; i < (*polynomial)->degree; i++) {
        long long value = (*polynomial)->data[i];
        printf("%lld ", value);
